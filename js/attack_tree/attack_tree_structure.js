...
(function () {
  ...
  function _atRefreshUIState() {
    const _safe = (fn, label) => {
      try {
        ...
        console.warn(`[AT Structure] ${label}:`, e.message || e);
      }
    };
    ...
    [1, 2].forEach((b) => {
      ['a', 'b'].forEach((g) => {
        _safe(
          () => _atUpdateAddImpactButtonState(b, g),
          `_atUpdateAddImpactButtonState(${b},'${g}')`
        );
        _safe(
          () => _atUpdateRemoveImpactButtonState(b, g),
          `_atUpdateRemoveImpactButtonState(${b},'${g}')`
        );
      });
    });
  ...
  function _atSetSecondIntermediateEnabled(enabled) {
    ...
    [1, 2].forEach((branchNum) => {
      ...
      document
        .querySelectorAll(`tr.branch-b-separator[data-branch="${branchNum}"]`)
        .forEach((r) => {
          if (!r) return;
          r.style.display = on ? '' : 'none';
        });
  ...
  function _atClearLeafFields(leafIdx) {
    ...
    document
      .querySelectorAll(`input[type="checkbox"][name^="at_leaf_${leafIdx}_ds"]`)
      .forEach((chk) => {
  ...
  function _atEnsureLeafDeleteButtons() {
    ...
    document.querySelectorAll('tr.impact-row').forEach((row) => {
      ...
      const _delTip =
        typeof t === 'function'
          ? t('at.impact.delete.titleTip')
          : 'Auswirkung löschen (mit Bestätigung)';
      btn.title = _delTip;
    ...
    btns.forEach((btn) => {
      btn.onclick = () => {
        ...
        try {
          ...
          console.warn('[AT] populateAttackTreeDropdowns:', e.message || e);
        }
        try {
          if (typeof updateAttackTreeKSTUSummariesFromForm === 'function')
            updateAttackTreeKSTUSummariesFromForm();
        } catch (e) {
          console.warn('[AT] updateSummaries:', e.message || e);
        }
      };
    });
  }
    ...
    btns.forEach((btn) => {
      btn.onclick = () => {
        ...
        if (!_atLeafDataIsEmpty(leafData)) {
          const ok = confirm(
            typeof t === 'function'
              ? t('at.impact.confirmNonEmpty')
              : 'Diese Auswirkung ist nicht leer. Wirklich löschen?'
          );
          if (!ok) return;
        }
