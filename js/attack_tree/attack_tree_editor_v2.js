...
(function () {
  ...
  function pasteInto(editor, targetNode) {
    if (!_clip) {
      _toast('Zwischenablage ist leer.', 'warning');
      return;
    }
  ...
  function renderNode(editor, parent, node, depth, crumb) {
    ...
    const btnCopy = _mkIconBtn('<i class="fas fa-copy"></i>', `${kindLabel} kopieren`, () => {
      _clip = { mode: 'copy', kind: 'node', payload: structuredClone(node) };
      _toast(`${kindLabel} kopiert.`, 'info');
    });
    ...
    const btnCut = _mkIconBtn(
      ...
      () => {
        ...
        _toast(`${kindLabel} ausgeschnitten – jetzt am Ziel einfügen.`, 'info');
      }
    );
    ...
    node.children.forEach((ch) =>
      childrenWrap.appendChild(
        renderNode(editor, node, ch, depth + 1, [...crumb, node.title || '(unbenannt)'])
      )
    );
  ...
  function renderImpact(editor, node, imp) {
    ...
    const impCopy = _mkIconBtn('<i class="fas fa-copy"></i>', 'Auswirkung kopieren', () => {
      _clip = { mode: 'copy', kind: 'impact', payload: structuredClone(imp) };
      _toast('Auswirkung kopiert.', 'info');
    });
    const impCut = _mkIconBtn(
      ...
      () => {
        ...
        _toast('Auswirkung ausgeschnitten – jetzt in Ziel-Pfad einfügen.', 'info');
      }
    );
