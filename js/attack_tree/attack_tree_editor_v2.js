...
(function () {
  ...
  function _confirm({ title, html, confirmText, confirmClass = "primary-button dangerous", onConfirm }) {
  ...
  function pasteInto(editor, targetNode) {
    if (!_clip) { _toast(_t("at.toast.clipEmpty"), "warning"); return; }
  ...
  function renderNode(editor, parent, node, depth, crumb) {
    ...
    const btnCopy = _mkIconBtn(
      ...
      () => { _clip = { mode: "copy", kind: "node", payload: structuredClone(node) }; _toast(_tf("at.toast.copied", { kind: kindLabel }), "info"); }
    );

    const btnCut = _mkIconBtn(
      '<i class="fas fa-cut"></i>',
      _tf("at.cut.tip", { kind: kindLabel }),
      () => { _clip = { mode: "cut", kind: "node", payload: node, sourceArr: parent.children }; _toast(_tf("at.toast.cut", { kind: kindLabel }), "info"); }
    );
    ...
    node.children.forEach((ch) => childrenWrap.appendChild(renderNode(editor, node, ch, depth + 1, [...crumb, node.title || _t("at.untitled")])));
  ...
  function renderImpact(editor, node, imp) {
    ...
    const impCopy = _mkIconBtn(
      ...
      () => { _clip = { mode: "copy", kind: "impact", payload: structuredClone(imp) }; _toast(_t("at.toast.impactCopied"), "info"); }
    );
    const impCut = _mkIconBtn(
      '<i class="fas fa-cut"></i>',
      _tf("at.cut.tip", { kind: impactKind }),
      () => { _clip = { mode: "cut", kind: "impact", payload: imp, sourceArr: node.impacts }; _toast(_t("at.toast.impactCut"), "info"); }
    );
    ...
    ds.innerHTML = `
      ...
      ${dsList.map((dsItem) => {
        ...
        const short = (typeof getLocalizedField === "function")
          ? (getLocalizedField(dsItem, "short", undefined, { fallback: true }) || dsItem.short || "")
          : (dsItem.short || "");
        const name = (typeof getLocalizedField === "function")
          ? (getLocalizedField(dsItem, "name", undefined, { fallback: true }) || dsItem.name || dsItem.id)
          : (dsItem.name || dsItem.id);
        const desc = (typeof getLocalizedField === "function")
          ? (getLocalizedField(dsItem, "description", undefined, { fallback: true }) || dsItem.description || "")
          : (dsItem.description || "");
    ...
    const _strideFallback = [
      {id:"S",name:"Spoofing (Identitätstäuschung)",name_en:"Spoofing (identity deception)",short:"S",description:"Kann sich ein Angreifer oder ein fremdes Gerät als vertrauenswürdiger Teilnehmer ausgeben, um Zugriff zu erhalten? (z.\u00a0B. ein gefälschtes Servicetool).",description_en:"Can an attacker or foreign device impersonate a trusted participant to gain access? (e.g. a forged service tool)."},
      {id:"T",name:"Tampering (Manipulation)",name_en:"Tampering (manipulation)",short:"T",description:"Können Daten, Parameter, Konfigurationen oder die Firmware auf dem Gerät oder während der Übertragung unbefugt verändert werden?",description_en:"Can data, parameters, configurations or firmware on the device or in transit be changed without authorisation?"},
      {id:"R",name:"Repudiation (Abstreitbarkeit)",name_en:"Repudiation (non-repudiation gap)",short:"R",description:"Können kritische Aktionen durchgeführt werden, ohne dass wir im Nachhinein nachweisen können, wer es war? (Fehlende oder manipulierbare Logs).",description_en:"Can critical actions be performed without us later being able to prove who did them? (Missing or tamperable logs)."},
      {id:"I",name:"Information Disclosure (Informationsenthüllung)",name_en:"Information Disclosure",short:"I",description:"Können schützenswerte Informationen (z.\u00a0B. Passwörter, Rezepturen, Kundendaten oder Know-how) von Unbefugten ausgelesen werden?",description_en:"Can sensitive information (e.g. passwords, recipes, customer data or know-how) be read by unauthorised parties?"},
      {id:"D",name:"Denial of Service (Dienstverweigerung)",name_en:"Denial of Service",short:"D",description:"Kann das System so sabotiert oder überlastet werden, dass es seine Funktion einstellt oder träge wird? (z.\u00a0B. Blockade der Steuerung).",description_en:"Can the system be sabotaged or overloaded so that it stops working or becomes sluggish? (e.g. blocking the controller)."},
      {id:"E",name:"Elevation of Privilege (Rechteausweitung)",name_en:"Elevation of Privilege",short:"E",description:"Kann ein Nutzer mit geringen Rechten (z.\u00a0B. Gast/Operator) Berechtigungen erlangen, die ihm nicht zustehen (z.\u00a0B. Admin-/Service-Rechte)?",description_en:"Can a low-privilege user (e.g. guest/operator) obtain permissions they should not have (e.g. admin/service rights)?"}
    ];
    ...
    strideWrap.innerHTML = `
      ...
      ${strideCats.map(cat => {
        ...
        const tipName = _escapeHtml((typeof getLocalizedField === "function")
          ? (getLocalizedField(cat, "name", undefined, { fallback: true }) || cat.name || cat.id)
          : (cat.name || cat.id));
        const tipDesc = _escapeHtml((typeof getLocalizedField === "function")
          ? (getLocalizedField(cat, "description", undefined, { fallback: true }) || cat.description || "")
          : (cat.description || ""));
    ...
    return {
      ...
      open(existingEntry) {
        ...
        if (existingEntry) {
          ...
          const en = existingEntry.rootName_en != null
        ...
        if (rootInput) {
          ...
          rootInput.oninput = () => {
            ...
            try { this.updateBreadcrumbs(); } catch (_) {}
          };
          _syncHint(rootInput, this.root, "title", rootPh);
        }
      ...
      updateBreadcrumbs() {
        const displayRoot = (typeof getLocalizedField === 'function')
