...
function generateDotString(analysis, specificTreeId = null) {
  ...
  const _emitDotTreeV2 = (entry) => {
    ...
    nodes.push(
      `    ${rootId} [label="${_lbl(entry.rootName, entry.kstu, entry.i_norm)}", style=filled, fillcolor="${rootFill}"]\n`
    );
    pushRank(0, rootId);
    ...
    const walk = (node, depth) => {
      ...
      nodes.push(
        `    ${nid} [label="${_lbl(node.title, node.kstu, node.i_norm)}", style=filled, fillcolor="${fill}"]\n`
      );
      pushRank(depth, nid);
      ...
      (node.impacts || []).forEach((leaf, idx) => {
        ...
        nodes.push(
          `    ${lid} [label="${_lbl(leaf.text, lkstu, leaf.i_norm, leaf.stride)}", style=filled, fillcolor="${lfFill}"]\n`
        );
        impactIds.push(lid);
        leafIds.push(lid);
      });
