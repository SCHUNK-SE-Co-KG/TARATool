...
function _applyAssessmentConfig(parsed, sourceLabel) {
  ...
  console.log(
    `[config_loader] Assessment config v${parsed._meta?.version || '?'} loaded from ${_lastConfigSource}`
  );
...
function reloadAssessmentConfigFromObject(parsed, sourceLabel) {
  ...
  if (typeof onAssessmentConfigReloaded === 'function') {
    try {
      onAssessmentConfigReloaded();
    } catch (e) {
      console.warn('[config_loader] UI refresh:', e);
    }
  }
  return true;
}
...
function reloadAssessmentConfigFromJsonText(jsonText, sourceLabel) {
  try {
    ...
    if (typeof showToast === 'function')
      showToast(
        typeof tf === 'function'
          ? tf('toast.configInvalidJson', { msg: e.message || e })
          : 'Ungültige JSON-Datei: ' + (e.message || e),
        'error'
      );
    return false;
  }
...
function loadAssessmentConfig() {
  try {
    if (
      typeof window.__ASSESSMENT_CONFIG_PRELOAD__ !== 'undefined' &&
...
if (!_configLoaded) {
  ...
  console.warn(
    '[config_loader] Nutze auf der Übersicht „Bewertungsconfig laden“ oder tools/sync_assessment_config.sh / .bat / .py'
  );
} else {
  window.ASSESSMENT_CONFIG = ASSESSMENT_CONFIG;
}
...
function taraConfigStatus() {
  ...
  const sOpts =
    typeof PROBABILITY_CRITERIA !== 'undefined' && PROBABILITY_CRITERIA.S
