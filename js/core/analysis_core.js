...
function createNewAnalysis(e) {
  ...
  if (!newAnalysis.metadata)
    newAnalysis.metadata = { version: INITIAL_VERSION, author: 'Unbekannt', date: today };
...
function deleteActiveAnalysis() {
  ...
  if (msg)
    msg.innerHTML = `Sind Sie sicher, dass Sie die Analyse <strong>${escapeHtml(analysis.name)}</strong> unwiderruflich löschen möchten? <br><br><span style="color:red;">Warnung: Alle Assets, Schadensszenarien und Angriffsbäume gehen verloren!</span>`;
