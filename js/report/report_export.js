  ...
  async function generateReportPdf() {
    ...
    if (risks.length > 0) {
      ...
      for (let ti = 0; ti < sortedTrees.length; ti++) {
        ...
        doc.setTextColor(0);
        doc.text(
          L.attackTree + ' ' + (entry.id || '') + ': ' + (entry.rootName || ''),
          treeMargin,
          treeMargin
        );
        ...
        if (svgText && svgText.includes('<svg')) {
          ...
          try {
            png = await h.svgTextToPng(svgText, targetPxW);
          } catch (e) {
            png = null;
          }
          if (png && png.dataUrl) {
    ...
    if (!rrEntries || rrEntries.length === 0 || typeof rrIterateLeaves !== 'function') {
      ...
      rrEntries.forEach((rrEntry) => {
        ...
        const rootName = h.sanitizePdfText(
          (rrEntry.rootName || origEntry?.rootName || rrEntry.name || rrEntry.root || '').toString()
        );
    ...
    if (risks.length > 0) {
      ...
      for (let ti = 0; ti < sortedTrees.length; ti++) {
        ...
        doc.setTextColor(0);
        doc.text(
          L.residualTree + ' ' + (entry.id || '') + ': ' + (entry.rootName || ''),
          rrMargin,
          rrMargin
        );
        doc.setFont('helvetica', 'normal');
        ...
        if (rrSvgText && rrSvgText.includes('<svg')) {
          ...
          try {
            png = await h.svgTextToPng(rrSvgText, targetPxW);
          } catch (e) {
            png = null;
          }
          if (png && png.dataUrl) {
