...
def test_github_board_doc_contains_project_id():
    ...
    content = open(os.path.join(REPO_ROOT, 'docs', 'GITHUB_BOARD.md')).read()
...
def test_onboarding_doc_contains_prerequisites():
    ...
    content = open(os.path.join(REPO_ROOT, 'docs', 'DEV_AGENT_ONBOARDING.md')).read()
...
def test_tests_readme_contains_noconftest():
    ...
    content = open(os.path.join(REPO_ROOT, 'tests', 'README.md')).read()
